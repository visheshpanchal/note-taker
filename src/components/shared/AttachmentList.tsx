...
import type { AnyNote, Attachment } from '../../types'
...
export function AttachmentList({ note, updateNote }: AttachmentListProps) {
  ...
  async function addFiles(filePaths: string[]) {
    ...
    try {
      ...
      if (result.success && result.added.length) {
        updateNote(note.id, { attachments: [...attachments, ...result.added] } as Partial<AnyNote>)
      } else if (!result.added?.length) {
        setError('Could not attach file. The file may be too large (100 MB limit) or inaccessible.')
      }
    } catch (e) {
      setError('Failed to attach file: ' + ((e as Error)?.message || 'unknown error'))
    } finally {
      setAdding(false)
    }
  }
  ...
  async function handlePickFiles() {
    if (!hasElectronAPI) { setError('Attachments are only available in the desktop app.'); return }
    try {
      const paths = await window.electronAPI!.attachments.pickFiles()
      if (paths.length) addFiles(paths)
    } catch (e) {
      setError('Could not open file picker: ' + ((e as Error)?.message || 'unknown error'))
    }
  }

  async function handleRemove(att: Attachment) {
    await window.electronAPI!.attachments.remove(note.id, att.filename)
    updateNote(note.id, { attachments: attachments.filter(a => a.id !== att.id) } as Partial<AnyNote>)
  }
  ...
  return (
    <div className="attachment-list">
      <div className="attachment-list__header">
        ...
        <button className="attachment-list__add-btn" onClick={handlePickFiles} disabled={adding}>
          {adding ? '⏳' : '+ Add file'}
        </button>
      </div>
          ...
          {attachments.map(att => (
            <div key={att.id} className="attachment-card">
              ...
              <div className="attachment-card__actions">
                <button className="attachment-card__open-btn" onClick={() => handleOpen(att)} title="Open">↗</button>
                <button className="attachment-card__del-btn" onClick={() => handleRemove(att)} title="Remove">×</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {error && <div className="attachment-error" onClick={() => setError(null)}>⚠ {error}</div>}
