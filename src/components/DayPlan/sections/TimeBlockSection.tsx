...
import './Sections.css'
import type { TimeBlockSectionData, TimeBlock } from '../../../types'

interface TimeBlockSectionProps {
  section: TimeBlockSectionData
  onUpdate: (patch: Partial<TimeBlockSectionData>) => void
  isToday: boolean
}

const CATEGORIES = [null, 'work', 'personal', 'health', 'learning', 'break'] as const
const CAT_LABELS: Record<string, string> = {
  null: 'None', work: 'Work', personal: 'Personal', health: 'Health', learning: 'Learning', break: 'Break'
}

function currentTimeBlock(items: TimeBlock[]): string | null {
  const now = new Date()
...
export function TimeBlockSection({ section, onUpdate, isToday }: TimeBlockSectionProps) {
  const items = section.items ?? []
  const currentId = useMemo(() => isToday ? currentTimeBlock(items) : null, [isToday, items])

  function updateItem(id: string, patch: Partial<TimeBlock>) {
    onUpdate({ items: items.map(i => i.id === id ? { ...i, ...patch } : i) })
  }

  function cycleCategory(item: TimeBlock) {
    const idx = CATEGORIES.indexOf(item.category as typeof CATEGORIES[number])
        ...
        {items.map(item => (
          ...
          >
            ...
            <button
              ...
              title={`Category: ${CAT_LABELS[item.category ?? 'null']}`}
            />
