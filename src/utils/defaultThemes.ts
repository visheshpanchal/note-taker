...
// ─── Versioning ───────────────────────────────────────────────────────────────
//
// Bump this when the token set changes so ThemeContext can migrate older themes.
//
// Version history:
//   1 — initial token set (22 tokens: bg/text/border/accent/state/shadow)
//   2 — added --transition-slow; ALL_TOKEN_KEYS now covers radius, sidebar-width,
//       transition, font stacks so they are properly cleared on theme switch.
//
export const CURRENT_SCHEMA_VERSION = 2

// ─── Mode defaults ────────────────────────────────────────────────────────────
//
// Mirrors :root / [data-theme=dark] in index.css exactly.
// These are the fallback values used when migrating older themes that are
// missing tokens introduced in later schema versions.
//
const SHARED_TOKENS: Record<string, string> = {
  ...
  '--font-mono':      '"SF Mono", "Menlo", "Consolas", "JetBrains Mono", monospace',
}

export const MODE_DEFAULTS: Record<'light' | 'dark', Record<string, string>> = {
  light: {
    '--bg-primary':    '#ffffff',
    '--bg-secondary':  '#f4f4f8',
    '--bg-tertiary':   '#eaeaef',
...
// Derived: every token key across both modes (union, deduped).
// ThemeContext uses this to know which inline style props to clear on switch.
export const ALL_TOKEN_KEYS = Array.from(
  new Set([
...
export const BUILT_IN_THEMES: Theme[] = [
  ...
  },
  {
    ...
    tokens: {
      ...
      '--bg-sidebar':    '#f2e8d6',
      '--text-primary':  '#2c1a0e',
      '--text-secondary':'#6b4c2e',
      '--text-muted':    '#a0816a',
      '--border':        '#e0d0b8',
      '--border-strong': '#c8b090',
      '--accent':        '#a0522d',
      '--accent-hover':  '#8b3d1a',
      '--accent-fg':     '#ffffff',
